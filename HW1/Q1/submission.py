...
class Graph:
    ...
    def max_degree_nodes(self) -> dict:
        ...
        for item in self.edges:
            for element in item:
                if element not in degree_dict:
                    degree_dict[element] = 1
                else:
                    degree_dict[element] += 1
        return {key: value for key, value in degree_dict.items() if value == max(degree_dict.values())}
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        if 'cast' in movie_cast:
            for item in movie_cast['cast']:
                if item['id'] not in exclude_ids:
                    if (limit is not None) and item['order'] >= limit:
                        continue
                    if ',' in item['name']:
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        r2 = conn.getresponse()
        movie_credit = json.loads(r2.read())
        
        credit = []
        if 'cast' in movie_credit:
            for item in movie_credit['cast']:
                if (vote_avg_threshold is not None) and item['vote_average'] < vote_avg_threshold:
                    continue
                credit.append(item)
...
def return_argo_lite_snapshot()->str:
    ...
    """
    return 'https://poloclub.github.io/argo-graph-lite/#fcf29525-9905-456c-9553-6b44e7bd0f22'
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='a6f14e4e42c4333eb1d45c9ddd2009e4')
    ...
    creds = tmdb_api_utils.get_movie_credits_for_person('2975', 8.0)
    Topcast = []
    for cred in creds:
        Topcast.append(tmdb_api_utils.get_movie_cast(str(cred['id']), 3))
    ...
    for film in Topcast:
         for item in film:
             if item['id'] not in nodes:
                 nodes.append(str(item['id']))
                 graph.add_node(str(item['id']), str(item['name']))
                 graph.add_edge('2975', str(item['id']))
    
                 
    for node in nodes[1:]:
        best = tmdb_api_utils.get_movie_credits_for_person(str(node), 8.0)
        for item in best:
            castmem = tmdb_api_utils.get_movie_cast(str(item['id']), 3)
            Topcast.append(castmem)
            for mem in castmem:
                if mem['id'] not in nodes:
                    nodes.append(mem['id'])
                    graph.add_node(str(mem['id']), str(mem['name']))
                    graph.add_edge(str(node), str(mem['id'])) 
                else:
                    graph.add_edge(str(node), str(mem['id'])) 
