...
class HW2_sql():
    ...
    def part_aii_1(self,connection,path):
        ...
        with open(path, encoding="utf8") as file:
            data_movie = csv.reader(file, delimiter = ',')
            for row in data_movie:
    ...
    def part_aii_2(self,connection, path):
        ############### CREATE IMPORT CODE BELOW ############################
        with open(path, encoding="utf8") as file:
            data_cast = csv.reader(file, delimiter = ',')
            for row in data_cast:
    ...
    def part_c(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        # part_c_sql = "select count(score) from movies where score > 50 and title like '%war%'/select count(*) from movies"
        part_c_sql = "select printf('%0.02f',100.*((select count(*) from movies where score > 50 and title like '%war%')*1.00/(select count(*) from movies)))"
        ######################################################################
    ...
    def part_e(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_e_sql = "select movies.title as movie_title, printf('%.2f',1.0*movies.score) as movie_score, count(movie_cast.movie_id) as cast_count from movies inner join movie_cast on movies.id = movie_cast.movie_id group by movies.title order by movies.score desc, cast_count asc, movies.title limit 5"
        ######################################################################
    ...
    def part_f(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_f_sql = "select movie_cast.cast_id, movie_cast.cast_name, printf('%.2f', 1.0*avg(movies.score)) as average_score from movie_cast inner join movies on movies.id = movie_cast.movie_id where movies.score>=25 group by movie_cast.cast_id having count(movie_cast.movie_id)>2 order by average_score desc, movie_cast.cast_name limit 10"
        ######################################################################
    ...
    def part_g(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_sql = "create view good_collaboration as select cast1.cast_id as cast_member_id1, cast2.cast_id as cast_member_id2, count(movies.id) as movie_count, print('%.2f',1.0*avg(movies.score)) as average_movie_score from movie_cast as cast1 inner join movie_cast as cast2 on cast1.movie_id=cast2.movie_id inner join movies on cast1.movie_id = movies.id where cast1.cast_id < cast2.cast_id group by cast1.cast_id, cast2.cast_id having movie_count >= 3 and average_movie_score >=40" 
        ######################################################################
