...
var Schema_taxi = StructType(Array(StructField("LocationID", IntegerType, true), StructField("Borough", StringType, true), StructField("Zone", StringType, true), StructField("service_zone", StringType, true)))
...
// ENTER THE CODE BELOW
val top_DO_loc = df_filter.groupBy($"DOLocationID").agg(count($"DOLocationID") as "number_of_dropoffs").orderBy($"number_of_dropoffs".desc, $"DOLocationID".asc).limit(5)
...
// ENTER THE CODE BELOW
val top_PU_loc = df_filter.groupBy($"PULocationID").agg(count($"PULocationID") as "number_of_pickups").orderBy($"number_of_pickups".desc, $"PULocationID".asc).limit(5)
...
// ENTER THE CODE BELOW
var DO_loc = df_filter.groupBy($"DOLocationID").agg(count($"DOLocationID") as "number_of_dropoffs").orderBy($"number_of_dropoffs".desc, $"DOLocationID".asc)

var PU_LOC = df_filter.groupBy($"PULocationID").agg(count($"PULocationID") as "number_of_pickups").orderBy($"number_of_pickups".desc, $"PULocationID".asc)

val comb = DO_loc.join(PU_LOC, $"DOLocationID" === $"PULocationID")                
                .withColumn("LocationID", col("DOLocationID"))
                .withColumn("number_activities" , col("number_of_dropoffs")+ col("number_of_pickups"))
                .orderBy($"number_activities".desc,$"LocationID".asc).select($"LocationID",$"number_activities").limit(3)
...
// ENTER THE CODE BELOW
val part3 = comb_temp.join(df_lookup, df_lookup("LocationID") === comb_temp("LocationID"))
                    .groupBy($"Borough")
...
// ENTER THE CODE BELOW
val part4 = df_filter.select($"pickup_datetime")
                  ...
                  .groupBy("day_of_week").agg(avg($"count") as "avg_count")
                  .orderBy($"avg_count".desc).limit(2)
...
// ENTER THE CODE BELOW
val part5_temp = df_filter.join(df_lookup, $"PULocationID" === $"LocationID").select($"PULocationID",$"LocationID",$"pickup_datetime",$"Borough",$"Zone")
                         .withColumn("hour",hour(col("pickup_datetime")))
                         .filter($"Borough" === "Brooklyn")
                         .groupBy("hour","zone").agg(count($"hour") as "count")
                         .orderBy($"hour".asc)
                         .groupBy("hour").agg(max($"count") as "max_count")
                         .withColumn("hour_of_day",col("hour"))
                         .drop($"hour")


val part5 = part5_temp.join(df_5_temp,$"hour" === $"hour_of_day" && $"count" === $"max_count")
...
val part6 = df_filter.select($"pickup_datetime",$"PULocationID")
                        ...
                        .withColumn("prior day count", prior_day)
